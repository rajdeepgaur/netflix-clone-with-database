...
export function BrowseContainer({ slides }){
    ...
    const setTrailer = async(item) => {
        movieTrailer(item.title, ( error, response ) => {
            if(error) {
                return;
            }
            setTrailerUrl(response.substring(response.indexOf("=") + 1));
        } )
      }
       ...
       <Card.Group>
           {slideRows.map((slideItem) => (
               <Card key={`${category}-${slideItem.title.toLowerCase()}`}>
                  ...
                  <Card.Entities>
                      {slideItem.data.sort(() => Math.random() - 0.5) && slideItem.data.map((item) => ( 
                         <Card.Item key={item.docId} item={item} setTrailer={setTrailer}>
